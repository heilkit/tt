...
env GOOS=linux GOARCH=386 go build -ldflags '-extldflags "-static"' -tags netgo,osusergo -o target/tikmeh_linux_i386_"${DATE}" ./cmd
...
env GOOS=windows GOARCH=amd64 go build -ldflags '-extldflags "-static"' -tags netgo,osusergo -o target/tikmeh_windows_x64_"${DATE}.exe" ./cmd
env GOOS=windows GOARCH=386 go build -ldflags '-extldflags "-static"' -tags netgo,osusergo -o target/tikmeh_windows_x86_"${DATE}.exe" ./cmd
